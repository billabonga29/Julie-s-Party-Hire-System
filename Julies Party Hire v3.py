...
#Check if receipt number exists in file
def validate_receipt_number():
    ...
    if os.path.exists("hire_details.txt"):
        with open("hire_details.txt", "r") as file:
            ...
            for line in lines:
                stored_receipt_number = line.split(",")[1]
...
#Pop-up window to find which customer is returning
def open_return_window():
    ...
    receipt_number_entry = tk.Entry(enter_receipt_root, width=10, font=("Comic Sans MS", 12), validate="key", validatecommand=(root.register(validate_numerical_input), "%P"))
