    ...
    def submit_hire():
        ...
        try:
            ...
            if hired_amount < 1 or hired_amount > 100:
                output_text.delete("1.0", tk.END)
                output_text.insert(tk.END, "ERROR! You can only hire between 1-100 items.\n")
                return
        except ValueError:
            output_text.delete("1.0", tk.END)
            output_text.insert(tk.END, "ERROR! Amount hired must be a valid number.\n")
            return
...
def return_window(receipt_number, hire_details):
    ...
    return_label = tk.Label(return_root, text="Return", font=("Bodoni MT Black", 24), bg="#B0C4DE")
